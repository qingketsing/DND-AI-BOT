...
// ChatRequest 定义发往 OpenAI 聊天补全接口的最小请求结构。
...
// ChatResponse 定义 OpenAI 聊天补全接口的最小响应结构。
...
// Client 封装 OpenAI 聊天补全请求所需的底层 HTTP 细节。
type Client struct {
	baseURL    string
	apiKey     string
...
// NewClient 创建一个可复用的 OpenAI HTTP 客户端。
...
// ChatCompletion 调用 OpenAI 的聊天补全接口并返回解码后的响应。
func (c *Client) ChatCompletion(ctx context.Context, request ChatRequest) (ChatResponse, error) {
	...
	if err != nil {
		return response, fmt.Errorf("execute openai request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		body, _ := io.ReadAll(resp.Body)
		return response, fmt.Errorf("openai request failed with status %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
