...
// BuildChatRequest 将统一模型输入转换为 OpenAI 的聊天补全请求。
...
// ParseChatResponse 将 OpenAI 聊天补全响应转换为 Runtime 输出。
func ParseChatResponse(response ChatResponse) (runtime.ModelOutput, error) {
	if len(response.Choices) == 0 {
		return runtime.ModelOutput{}, fmt.Errorf("openai response has no choices")
	}
	...
	if message.Content == "" {
		return runtime.ModelOutput{}, fmt.Errorf("openai response content is empty")
	}
