...
// Adapter 将 Runtime 的统一模型输入映射到 DeepSeek 的聊天补全协议。
type Adapter struct {
	client *Client
	model  string
}

// NewAdapter 创建一个基于 DeepSeek 的模型适配器。
...
// Run 执行一次统一模型请求，并将 DeepSeek 响应映射回 Runtime 输出。
func (a *Adapter) Run(ctx context.Context, input runtime.ModelInput) (runtime.ModelOutput, error) {
	...
	if err != nil {
		return runtime.ModelOutput{}, fmt.Errorf("deepseek chat completion: %w", err)
	}
